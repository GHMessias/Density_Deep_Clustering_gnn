...
def default_config_path() -> Path:
    return PROJECT_ROOT / "configs" / "cora_ollivier_ricci_community.yaml"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compatibility wrapper for the YAML-driven Cora + Ollivier-Ricci community benchmark.",
    )
    parser.add_argument(
        "--config",
...
def main() -> None:
    ...
    print(f"Connected components: {metrics['ricci_num_connected_components']:.0f}")
    print(f"Mean cutoff: {metrics['ricci_mean_cutoff']:.4f}")
