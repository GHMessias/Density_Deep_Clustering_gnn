...
def default_config_path() -> Path:
    return PROJECT_ROOT / "configs" / "cora_arga_kmeans.yaml"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compatibility wrapper for the YAML-driven Cora + ARGA + KMeans benchmark.",
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override the random seed used by ARGA and KMeans.",
    )
    return parser
...
def main() -> None:
    ...
    print(f"ARGA discriminator loss: {metrics['arga_discriminator_loss']:.4f}")
