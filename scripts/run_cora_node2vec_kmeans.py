...
def default_config_path() -> Path:
    return PROJECT_ROOT / "configs" / "cora_node2vec_kmeans.yaml"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compatibility wrapper for the YAML-driven Cora + Node2Vec + KMeans benchmark.",
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override the random seed used by Node2Vec and KMeans.",
    )
    return parser
...
def main() -> None:
    ...
    print(f"Node2Vec loss: {metrics['node2vec_loss']:.4f}")
